...
describe('在长度 2N 的数组中找出重复 N 次的元素', () => {
    ...
    it('数组长度为 4', () => {
        expect(repeatedNTimes([6, 6, 2])).toBe(6)
        expect(repeatedNTimes([6, 2, 6])).toBe(6)
        expect(repeatedNTimes([2, 6, 6])).toBe(6)

        expect(repeatedNTimes([6, 6, 6])).toBe(6)

        expect(repeatedNTimes([1, 2, 3])).toBe(-1)
    })
})
